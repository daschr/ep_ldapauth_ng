...
MyLdapAuth.prototype.close = function (cb) {
  ...
  if (typeof(self._adminClient) !== 'undefined') {
    self._adminClient.unbind(function (err) {
      if (err) {
        return cb(util.format('ldap unbind failed: %s', err));
      }
...
MyLdapAuth.prototype._adminBind = function (cb) {
  ...
  if (typeof(self.opts.adminDn) !== 'undefined') {
	console.debug("binding with %s and pw: %s", self.opts.adminDn, self.opts.adminPassword);
	self._adminClient.bind(self.opts.adminDn, self.opts.adminPassword,
	  ...
	  console.debug("admin bind success");
	  return cb();
    });
  }
  else {
...
MyLdapAuth.prototype._findUser = function (username, cb) {
  ...
  self._adminBind(function (err) {
    ...
    self._adminClient.search(self.opts.searchBase, opts,
      function (err, result) {
      if (err) {
        return cb(util.format('ldap search with filter %s and base %s failed: %s', searchFilter, self.opts.searchBase, err));
      }
      var items = [];
      ...
      result.on('error', function (err) {
        return cb(util.format('ldap search with filter %s and base %s returned error: %s', searchFilter, self.opts.searchBase, err));
      });
...
MyLdapAuth.prototype.authenticate = function (username, password, cb) {
  ...
  self._findUser(username, function (err, user) {
    ...
    userClient.bind(user.dn, password, function (err) {
      if (err) {
        return cb(util.format('password verify failed. could not bind dn %s: %s', user.dn, err));
      }
  ...
  self._adminBind(function (err) {
	...
	self._adminClient.search(self.opts.groupSearchBase, opts,
      ...
      if (err) {
        ...
        return cb(util.format('ldap group search with filter %s and base failed: %s', self.opts.groupSearch, self.opts.groupSearchBase, err));
      }
      if (!result)
      ...
      result.on('error', function (err) {
        ...
        return cb(util.format('ldap group search with filter %s and base returned error: %s', self.opts.groupSearch, self.opts.groupSearchBase, err));
      });
      result.on('end', function (result) {
        if (result.status !== 0) {
          var err = 'non-zero status from LDAP search: ' + result.status;
		...
		items.forEach(function (e){
			if(	typeof e[self.opts.groupAttribute] === 'string'){ 
				if(e[self.opts.groupAttribute] == usersearch)
					user_groups.push(e);
			}else if(e[self.opts.groupAttribute].some(function (i){
					return i==usersearch;
			})){
				user_groups.push(e);
			}
